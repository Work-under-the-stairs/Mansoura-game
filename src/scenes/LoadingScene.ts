...
export class LoadingScene {
  ...
  private currentProgress: number = 0; // Actual target from LoadingManager
  private visualProgress: number = 0;  // What the user actually sees
  ...
  private showTapToStart(): void {
    ...
    startScreen.innerHTML = `
      <style>
        #tap-to-start {
          ...
          background: #0a0f19; font-family: 'Cairo', sans-serif;
          cursor: pointer; direction: rtl;
        }
        #tap-to-start .tap-logo {
          color: #e8c97a; font-size: 32px; font-weight: 900;
        ...
        #tap-to-start .tap-icon svg { width: 20px; height: 20px; fill: #e8c97a; }
        #tap-to-start .tap-sub {
          color: rgba(255,255,255,0.4); font-size: 13px;
          letter-spacing: 4px; animation: tapPulse 1.6s ease-in-out infinite;
        }
        @keyframes tapPulse {
    ...
    const unlock = () => {
      ...
      // Start the smooth visual animation loop
    ...
    startScreen.addEventListener('touchstart', unlock, { once: true });
  }
  ...
  private initMusic(): void {
    ...
    try {
      ...
      this.audio.volume = 0;
      this.audio.play().then(() => this.fadeIn()).catch((err) => {
    ...
    el.innerHTML = `
      <style>
        #loading-screen {
          ...
          padding-bottom: 80px; font-family: 'Cairo', sans-serif;
        ...
        #ls-gradient {
          position: absolute; inset: 0;
          background: linear-gradient(to bottom, transparent 30%, rgba(10,15,25,0.85) 100%);
          z-index: 1;
        }
        #ls-content {
        ...
        #ls-title {
          color: #e8c97a; font-size: 28px; font-weight: 900;
          letter-spacing: 4px; text-align: center;
          text-shadow: 0 0 30px rgba(232,201,122,0.4); margin: 0;
        }
        #ls-subtitle {
          color: rgba(255,255,255,0.5); font-size: 12px;
          letter-spacing: 6px; margin: -8px 0 0;
        }
        #ls-bar-wrap { width: 100%; display: flex; flex-direction: column; gap: 8px; }
        #ls-bar-labels { display: flex; justify-content: space-between; }
        .ls-bar-label { color: rgba(255,255,255,0.4); font-size: 10px; letter-spacing: 3px; }
        #ls-pct { color: #e8c97a; font-size: 13px; font-weight: 700; }
        #ls-bar-outer {
        ...
        #ls-fill {
          ...
          /* No transition here, handled by RequestAnimationFrame */
        }
        #ls-fill::after {
          content: ''; position: absolute; right: -1px; top: -4px;
          width: 2px; height: 14px; background: #ffffff;
          border-radius: 1px; box-shadow: 0 0 8px #ffffff;
        }
        #ls-diamonds { display: flex; gap: 10px; align-items: center; margin-top: 4px; }
        .ls-line { width: 40px; height: 1px; background: rgba(232,201,122,0.25); }
        .ls-diamond {
          width: 6px; height: 6px; background: #e8c97a;
          transform: rotate(45deg); opacity: 0.2; transition: opacity 0.3s;
        }
        .ls-diamond.active { opacity: 1; }
        #ls-tip { color: rgba(255,255,255,0.3); font-size: 12px; text-align: center; }
  ...
  private animateProgressBar(): void {
    if (!this.progressBar || !this.progressText || !this.tipText) {
       this.animationId = requestAnimationFrame(() => this.animateProgressBar());
       return;
    }

    // Increment visual bar toward current actual progress
    // If currentProgress is 100, this will still take time to reach 100
    if (this.visualProgress < this.currentProgress) {
      this.visualProgress += 0.7; // Speed of the bar (0.7% per frame)
    }

    const clamped = Math.min(this.visualProgress, 100);
    ...
    if (clamped >= 100 && this.loadingComplete) {
